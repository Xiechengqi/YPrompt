...
YPrompt 是一个双端协同的提示词管理系统：前端采用 Vue 3 + Pinia + Tailwind 构建响应式 UI，后端由 FastAPI 提供基于 SQLite 的 API 服务，支持本地认证。此文档定义了各角色 Agent 的工作范围、关键入口以及协作方式，便于后续二次开发快速分工。

## 项目快照
- **目录结构**：`frontend/` 承载 Vue 应用（模块化组件 + composables）；`backend/` 承载 FastAPI 应用（apps/modules/* 路由模块化）；`data/` 保存 SQLite、日志与缓存。
- **运行链路**：浏览器命中 `frontend/src/main.ts` 配置的路由和守卫，认证后通过 `frontend/src/services/apiService.ts` 调用后端 REST 接口，后端在 `backend/main.py` 中初始化数据库/JWT/路由并暴露 `/api/**`。
...
- **关键工作流**：
  1. 认证：`authStore` 负责本地用户名密码登录、token 刷新、用户恢复。
...
### 后端 Agent
- **职责**：
 - 维护 FastAPI 应用生命周期（`backend/main.py`），包括日志、CORS、JWT、数据库、路由注册。
 - 编写/维护业务模块（`backend/apps/modules/*`），每个模块包含 `models.py`（Pydantic 模型）、`services.py`、`views.py`。
  - 维护工具层（`backend/apps/utils/`）：数据库适配器、JWT、密码工具、认证中间件等。
...
- **核心能力点**：
  ...
  2. **本地认证**：`apps/modules/auth/views.py` 提供本地 `/api/auth/local/login` 入口；`PasswordUtil` & `JWTUtil` 提供支持。
  3. **提示词/版本/标签** 模块提供 REST API（`/api/prompts`, `/api/versions`, `/api/tags`）；`prompt_rules` 模块负责系统内置规则管理。
- **常用命令**：`python3 -m venv venv && source venv/bin/activate`、`pip install -r requirements.txt`、`python run.py`（使用 uvicorn 启动）。

### 协调/集成 Agent
- **职责**：
  - 定义前后端契约，确保 `apiService` 中的请求与后端返回的数据结构一致，如 `/api/prompts` 列表返回 `total/page/limit/items`。
  - 管理配置与部署：对接 `.env` / `config/base.py` / `config/settings.py`，保证环境变量（数据库、JWT、登录用户）在两个端正确暴露。
...
- **认证链路**：前端通过 `/api/auth/config` 获取登录用户名；本地模式通过 `/api/auth/local/login` 完成登录。
- **数据一致性**：提示词与版本模块需要维护 `current_version`、`total_versions`、版本快照，前端 Library/Version 面板依赖这些字段；标签模块依赖 `tags` 字段的逗号分隔格式。
- **配置安全**：永远不要提交真实密钥到 repo；本地调试使用 `.env.local` / `builtin-providers.json`；生产环境通过环境变量覆盖 `SECRET_KEY`、数据库、登录用户密码。
